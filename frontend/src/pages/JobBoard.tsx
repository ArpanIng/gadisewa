...
const initialJobs = [
  ...
  },
  {
    ...
    plate: "TRK-456",
    status: "progress",
    tags: ["Engine Diagnostic", "Tire Rotation"],
    date: "23/10/24",
  ...
  >
    ...
    <div className="flex flex-wrap gap-1.5 mb-3">
      {job.tags.map((tag: string) => (
        <span
          key={tag}
          className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-medium rounded border border-slate-200 dark:border-slate-700"
        >
          {tag}
        </span>
      ))}
    ...
    <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-700">
      ...
      <div className="flex items-center gap-2">
        ...
        <img
          src={job.avatar}
          alt="User"
          className="w-6 h-6 rounded-full border border-white dark:border-slate-600"
        />
      </div>
    </div>
  </Link>
    ...
    <div className="h-full flex flex-col">
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Track and manage all vehicle service jobs. Drag cards to update
            status.
          </p>
        </div>
        <button className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium flex items-center gap-2">
      ...
      <div className="flex gap-4 mb-6">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Search by Customer or Reg No."
            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
          />
        </div>
        ...
        <div className="flex gap-4 min-w-max h-full">
          {columns.map((col) => {
            ...
            return (
              <div
                ...
                className={`w-80 flex flex-col h-full rounded-xl transition-colors ${
                  dragOverColumn === col.id
                    ...
                    : "bg-slate-100 dark:bg-[#0d141c]/50"
                }`}
              ...
              >
                <div
                  className={`p-3 border-t-4 ${col.color} bg-white dark:bg-[#1e293b] rounded-t-xl shadow-sm mb-2 flex justify-between items-center`}
                >
